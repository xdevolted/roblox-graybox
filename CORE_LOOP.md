...
3. **Resolve:** The server declares success immediately after confirming safe-zone entry, or failure on timeout, death, or falling into the void. Duplicate or late terminal events cannot change the first result.
...
The server owns every state transition and outcome. Each player has an independent lifecycle, including players who join late. Headless rules model lifecycle events without Roblox objects; later adapters observe Roblox movement, character, time, and zone state.
